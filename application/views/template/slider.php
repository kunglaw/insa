<!-- BEGIN .main-slider -->
<div class="main-slider">

  <!-- BEGIN .wrapper -->
  <div class="wrapper">

      <div class="main-slider-owl">
          <div class="item">
              <a href="post.html" class="main-slider-owl-title">Fusce porttitor porta ante dicta legimus noluisse</a>
              <a href="blog.html" class="main-slider-owl-calendar"><strong><i class="material-icons">access_time</i>December 12, 2015</strong></a>
              <img src="<?=ASSET_URL."images/photos/image-1.jpg"?>" alt="" />
          </div>
          <div class="item">
              <a href="post.html" class="main-slider-owl-title">Fusce porttitor porta ante dicta legimus noluisse</a>
              <a href="blog.html" class="main-slider-owl-calendar"><strong><i class="material-icons">access_time</i>December 12, 2015</strong></a>
              <img src="<?=ASSET_URL."images/photos/image-23.jpg"?>" alt="" />
          </div>
          <div class="item">
              <a href="post.html" class="main-slider-owl-title">Fusce porttitor porta ante dicta legimus noluisse</a>
              <a href="blog.html" class="main-slider-owl-calendar"><strong><i class="material-icons">access_time</i>December 12, 2015</strong></a>
              <img src="<?=ASSET_URL."images/photos/image-24.jpg"?>" alt="" />
          </div>
          <div class="item">
              <a href="post.html" class="main-slider-owl-title">Fusce porttitor porta ante dicta legimus noluisse</a>
              <a href="blog.html" class="main-slider-owl-calendar"><strong><i class="material-icons">access_time</i>December 12, 2015</strong></a>
              <img src="<?=ASSET_URL."images/photos/image-25.jpg"?>" alt="" />
          </div>
          <div class="item">
              <a href="post.html" class="main-slider-owl-title">Fusce porttitor porta ante dicta legimus noluisse</a>
              <a href="blog.html" class="main-slider-owl-calendar"><strong><i class="material-icons">access_time</i>December 12, 2015</strong></a>
              <img src="<?=ASSET_URL."images/photos/image-24.jpg"?>" alt="" />
          </div>
          <div class="item">
              <a href="post.html" class="main-slider-owl-title">Fusce porttitor porta ante dicta legimus noluisse</a>
              <a href="blog.html" class="main-slider-owl-calendar"><strong><i class="material-icons">access_time</i>December 12, 2015</strong></a>
              <img src="<?=ASSET_URL."images/photos/image-25.jpg"?>" alt="" />
          </div>
      </div>

  <!-- END .wrapper -->
  </div>

<!-- BEGIN .main-slider -->
</div>